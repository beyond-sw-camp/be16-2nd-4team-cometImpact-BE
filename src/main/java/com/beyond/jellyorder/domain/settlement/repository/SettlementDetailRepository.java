...
public interface SettlementDetailRepository extends Repository<Sales, java.util.UUID> {
    ...
    @Query(value = """
        ...
        FROM unit_order uo
        JOIN total_order t          ON uo.total_order_id = t.id
        JOIN sales s                ON s.total_order_id = t.id
        ...
        WHERE ss.id = :storeId
          ...
          AND (:status IS NULL OR s.status = :status)  -- 'COMPLETED' 또는 'CANCELLED' 필터
        GROUP BY uo.id, COALESCE(s.paid_at, s.updated_at), s.payment_method, s.status
        ORDER BY COALESCE(s.paid_at, s.updated_at) DESC, uo.id
        """,
            countQuery = """
        SELECT COUNT(*) FROM (
            SELECT uo.id
            ...
            GROUP BY uo.id, COALESCE(s.paid_at, s.updated_at), s.status
        ) x
        """,
            nativeQuery = true)
    Page<Object[]> findUnitOrderHeaders(
    ...
    @Query(value = """
        ...
        WHERE uo.id IN (:unitOrderIds)
        ORDER BY uo.id, om.id
        """,
            nativeQuery = true)
    List<Object[]> findMenuLinesByUnitOrders(@Param("unitOrderIds") List<UUID> unitOrderIds);
}
